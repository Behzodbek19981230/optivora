...
const ConfirmationListTabApprove = ({ refresh }: { refresh: string }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await DataService.get(endpoints.applicationApprovedFilter, {
        ...
        page: pagination.page + 1,
        perPage: pagination.pageSize,
        baranch: user.user?.branch?.id,
        merchant: user.user?.branch?.merchantId
      })
      setApiData(res.data)
      setIsLoading(false)
    } catch (err: any) {}
...
export default ConfirmationListTabApprove
