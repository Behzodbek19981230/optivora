...
const TableHeader = (props: TableHeaderProps) => {
  ...
  const [merchant, setMerchant] = useState<number | null>(null)
      ...
      <Grid container spacing={2} sx={{ py: 4, px: 6 }}>
        ...
        <Grid item xs={4}>
          <Autocomplete
            ...
            onChange={(e, val) => {
              onSelectMerchant(val?.value)
            }}
            getOptionLabel={option => option.label}
            renderInput={params => <CustomTextField key={params.id} {...params} label={t('Merchants') as string} />}
          />
        </Grid>
        <Grid item xs={4}>
          <Autocomplete
        ...
        <Grid item xs={2} sx={{ display: 'flex', alignItems: 'end', textAlign: 'end' }}>
          <LoadingButton
            style={{ background: theme.palette.primary.contrastText }}
            variant='outlined'
