// ** React Imports
import { useState, ElementType, ChangeEvent, SyntheticEvent, useTransition } from 'react'
...
const TabContentBuyer = ({ tab, apiData }: any) => {
  ...
  const handleChange = (event: SyntheticEvent, value: string) => {
    ...
    router
      .push({
        pathname: `/application/${router.query?.id}/${value}`
      })
      .then(() => {
        setActiveTab(value)
        setIsLoading(false)
                ...
                <Box>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    <CustomChip
                      ...
                      skin='light'
                      size='small'
                      label={apiData?.pinfl ? t('verified_') : t('noverified_')}
                      color={apiData?.pinfl ? 'success' : 'error'}
                  ...
                  <Box sx={{ mr: 8, display: 'flex', alignItems: 'center' }}>
                    ...
                    <div>
                      <Typography sx={{ fontWeight: 500, color: 'text.secondary' }}>
                        {' '}
                        <CurrencyFormatter amount={apiData?.remaining as number} currency='sum' />
                      </Typography>
...
export default TabContentBuyer
