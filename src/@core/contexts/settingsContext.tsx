...
export const SettingsProvider = (props: Props) => {
  ...
  // Cookies (js-cookie based)
  const cookieName = themeConfig.settingsCookieName

  const getInitialCookieValue = (): Settings => {
...
return parsed && typeof parsed === 'object' ? parsed : updatedInitialSettings
      } catch {
    ...
    // Prefer server-provided cookie settings if available
    if (props.settingsCookie && JSON.stringify(props.settingsCookie) !== '{}') return props.settingsCookie
    
return updatedInitialSettings
  }

  const [settingsCookie, setSettingsCookie] = useState<Settings>(getInitialCookieValue())
  ...
  const updateSettings = (settings: Partial<Settings>, options?: UpdateSettingsOptions) => {
    ...
    _updateSettingsState(prev => {
      ...
      if (updateCookie) {
        Cookies.set(cookieName, JSON.stringify(newSettings))
